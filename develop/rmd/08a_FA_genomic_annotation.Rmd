...
We can see that the `grch37` object already contains all the information we want in a super easy way. Let's annotate the results of our shrunken DEA for Control vs Vampirium:
...
ids <- grch37 %>% dplyr::filter(ensgene %in% rownames(res_tableCont))
...
??? question "**Solution to Exercise 1**"
    ...
    ```{r}
    ids_grch38 <- grch38 %>% dplyr::filter(ensgene %in% rownames(res_tableCont))
    
    res_ids_grch38 <- inner_join(res_tableCont_tb, ids_grch38, by=c("gene"="ensgene"))
...
!!! question "**Exercise 2**"
    ...
    Annotate the results of your DEA for Garlicum vs Vampirium

??? question "**Solution to Exercise 2**"

    ```{r}
    ## Return the IDs for the gene symbols in the DE results
    ids <- grch37 %>% dplyr::filter(ensgene %in% rownames(res_tableGar))
