...
We can see that we have a coefficient for our *intercept* and coefficient for the effect of “treat” (i.e. differences between treat
versus control).
...
Now it’s harder to define contrasts between groups of patient of the same bloodtype using DESeq’s coefficient names (although still
possible). But using the model matrix approach, we do it in exactly the same way we have done so far!
