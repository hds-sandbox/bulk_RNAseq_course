...
knitr::include_graphics("./img/06a_count_matrix/rnaseq_full_workflow.png")
```
...
```{r, echo = FALSE, eval = TRUE}
knitr::include_graphics("./img/06a_count_matrix/settingup.png")
```

-   Inside the folder `Notebooks` you will find the scripts that we will follow during the sessions. They are both in `R Script` and `Rmd` formats. Choose whichever is more convenient for you, but we recommend to use `Rmd` files.

-   In the folder `Results` you will save the results of your scripts, analysis and tests.

To avoid copying the original dataset for each student (very inefficient) the dataset is contained inside the shared folder `/work/bulk_RNAseq_course/Data/`. Do not attempt to modify this folder, as it might mess up the files for the rest of your colleagues.
...
```{r}
## Load in data
data <- read_table("/work/bulk_RNAseq_course/Data/Mov10_full_counts.txt") 

meta <- read_table("/work/bulk_RNAseq_course/Data/Mov10_full_meta.txt", skip = 1, col_names = c("samplename","sampletype", "MOVexpr"))
```
...
```{r, echo = FALSE, eval = TRUE}
knitr::include_graphics("./img/06a_count_matrix/deseq_counts_overview.png")
```
...
```{r, echo = FALSE, eval = TRUE}
knitr::include_graphics("./img/06a_count_matrix/foldchange_heatmap.png")
```
...
```{r, echo = FALSE, eval = TRUE}
knitr::include_graphics("./img/06a_count_matrix/de_variation.png")
```
...
```{r, echo = FALSE, eval = TRUE}
knitr::include_graphics("./img/06a_count_matrix/de_norm_counts_var.png")
```
...
```{r, echo = FALSE, eval = TRUE}
knitr::include_graphics("./img/06a_count_matrix/deseq_counts_distribution.png")
```
...
```{r, echo = FALSE, eval = TRUE}
knitr::include_graphics("./img/06a_count_matrix/deseq_counts_distribution_zoomed.png")
```
...
```{r, echo = FALSE, eval = TRUE}
knitr::include_graphics("./img/06a_count_matrix/deseq_nb.png")
```
...
> **NOTE:** If you are using **cell lines** and are unsure whether or not you have prepared biological or technical replicates, take a look at [this link](https://web.archive.org/web/20170807192514/http://www.labstats.net:80/articles/cell_culture_n.html). This is a useful resource in helping you determine how best to set up your *in-vitro* experiment.
...
```{r, echo = FALSE, eval = TRUE}
knitr::include_graphics("./img/06a_count_matrix/deseq_mean_vs_variance.png")
```
...
The figure below illustrates the relationship between sequencing depth and number of replicates on the number of differentially expressed genes identified :
...
```{r, echo = FALSE, eval = TRUE}
knitr::include_graphics("./img/06a_count_matrix/de_replicates_img.png")
```
...
```{r, echo = FALSE, eval = TRUE}
knitr::include_graphics("./img/06a_count_matrix/deg_methods1.png")
```

```{r, echo = FALSE, eval = TRUE}
knitr::include_graphics("./img/06a_count_matrix/deg_methods2.png")
```
...
```{r, echo = FALSE, eval = TRUE}
knitr::include_graphics("./img/06a_count_matrix/deseq_workflow_full_2018.png")
```
