...
This is a bulk RNAseq analysis of the data set ID SRP008496.
# Load libraries
```{r}
library(DESeq2)
library(tidyverse)
...
```{r}
cts <- data.frame(read.csv(file = "../data/SRP008496_cts.tsv", sep = "\t", row.names = "ensembl_id"))
coldata <- data.frame(read.csv(file = "../data/SRP008496_metadata.tsv", sep = "\t", row.names = "ID"))
```
...
paste("The order of the conditions is:", paste(unique(coldata$group), collapse=", "))
```
...
```{r}
#coldata$group <- factor(coldata$group, levels = c("non_tumor","tumor"))
coldata$group <- relevel(coldata$group, ref = "non_tumor") #Specifies "non_tumor" as the reference level
```
...
In this example, we only have two conditions: "non_tumor" and "tumor", so there is no need to specify which comparison we would like to make.
...
```{r}
res <- results(dds, contrast=c("group","non_tumor","tumor"))
#head(data.frame(res))
```
...
```{r}
resultsNames(dds)
res <- results(dds, name="group_tumor_vs_non_tumor")
#head(data.frame(res))
```
...
df <- data.frame(row.names = rownames(colData(dds)),group = colData(dds)[,c("group")])
...
```{r}
resLFC <- lfcShrink(dds, coef="group_tumor_vs_non_tumor", type="apeglm")
