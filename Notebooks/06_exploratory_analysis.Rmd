...
By default `plotPCA()` uses the *top 500 most variable genes*. You can change this by adding the `ntop=` argument and specifying how many of the genes you want the function to consider. For example, try 1000 genes.
...
```{r}
sampleDists <- dist(t(assay(rld))) #distances are computed by rows, so we need to transpose the matrix
