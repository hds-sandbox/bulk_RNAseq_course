...
```{r}
# Load metadata
meta <- read_table("/work/bulk_RNAseq_course/Data/Mov10_full_meta.txt")
...
files <- file.path(dir,"salmon", meta$samplename, "quant.sf")
...
df <- data %>% 
  rowwise() %>% 
  summarise(mean_counts = mean(Mov10_kd_2:Control_3), 
            variance_counts = var(Mov10_kd_2:Control_3))
