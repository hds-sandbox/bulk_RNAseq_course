...
To load the data into our current environment, we will be using the `read.table` function. By default the function expects tab-delimited files, which is what we have.
...
```{r, echo = FALSE, eval = TRUE}
knitr::include_graphics("./img/06a_count_matrix/seqDepth_DEA.png")
```
